...
def get_layers_from_net(net, n_levels):
  ...
  for i in range(n_levels):
    ...
    for n in nodes:
      ...
      _next = net.successors(n)
      _dict[n_name] = [re.sub('_copy.*', '', nex) for nex in _next]
    layers.append(_dict)
  return layers
...
class Reactome:
  ...
  def __init__(self):
    self.pathway_names = load_names()
    self.hierarchy = load_hierarchy()
    self.pathway_genes = load_genes()
